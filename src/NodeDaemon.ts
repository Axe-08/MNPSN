...
import { Tx, Batch, MempoolDigest, NULL_BATCH, NodesRegistry, NodeConfig } from './types.js';
    ...
    this.slotManager.on('phase_changed', async (phase: Phase, slot: number) => {
      try {
        if (phase === 'FREEZE') {
          ...
          const localTxs = this.mempool.snapshot(slot);
          const localHashes = localTxs.map(tx => tx.hash);
          const excludedHashes = this.syncManager.getExcluded(localHashes, commonHashes);
